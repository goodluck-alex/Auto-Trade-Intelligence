...
const CompanyPage = () => {
  return (
    <div className="space-y-12 max-w-5xl mx-auto py-8">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div className="space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-purple-500/10 rounded-full border border-purple-500/20">
            ...
            <span className="text-[10px] font-black text-purple-600 uppercase tracking-widest">Our Story</span>
          </div>
        ...
        <div className="dark:bg-[#161723] bg-white border dark:border-white/5 border-gray-100 rounded-3xl p-8 shadow-sm grid grid-cols-2 gap-6">
           <VisionItem icon={<Target className="text-purple-600" />} label="Mission" desc="To democratize high-frequency futures trading." />
      ...
      <section className="space-y-8">
         <div className="flex items-center justify-between">
            <h3 className="text-xl font-black dark:text-white text-gray-900 uppercase tracking-wider flex items-center gap-2">
               <Newspaper size={20} className="text-purple-500" /> Blog & Insights
            </h3>
            <button className="text-[10px] font-black text-purple-600 uppercase tracking-widest hover:text-purple-700 transition-colors">View All Posts</button>
         </div>
      ...
      {/* Careers Section */}
      <section className="dark:bg-purple-900/10 bg-purple-50 border border-purple-500/20 rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-8">
         <div className="space-y-2">
            <h3 className="text-xl font-black dark:text-white text-gray-900 flex items-center gap-2">
               <Briefcase size={24} className="text-purple-600" /> We're Hiring
            </h3>
            <p className="text-sm text-gray-500">Join our engineering and research team in London or remotely.</p>
         </div>
         <button className="px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white font-black uppercase text-xs rounded-xl shadow-lg shadow-purple-500/20 transition-all">Explore Open Roles</button>
      </section>
    </div>
  );
};
...
const BlogPost = ({ title, date, category }: any) => (
  <div className="dark:bg-[#161723] bg-white border dark:border-white/5 border-gray-100 rounded-2xl p-6 hover:border-purple-500/50 transition-all group shadow-sm">
     <span className="text-[8px] font-black text-purple-600 uppercase tracking-widest block mb-2">{category}</span>
     <h4 className="text-sm font-bold dark:text-gray-200 text-gray-800 group-hover:text-purple-500 transition-colors mb-4">{title}</h4>
